...
std::string get_string(str_map const& map, std::string const& key,
...
struct dem_grid::impl {
  ...
  void init_hdr(str_map const& hdr) {
    ...
    //This implementation expects a positive YDIM
    ydim_ = std::abs(get_double(hdr, "YDIM", 0));
  ...
  pixel_value get(geo::latlng const& loc) {
    auto const lon = loc.lng();
...
elevation_t dem_grid::get(geo::latlng const& loc) const {
...
pixel_value dem_grid::get_raw(geo::latlng const& loc) const {
